...
function form_pools(
  ...
  case direction of
   ...
   (* A->B, B->C, A->c, vice versa for each*)
   ...
   Sell_a -> record [
    ...
    Buy_b -> record[
      token_a_pool = to_pool;
      token_c_pool = from_pool;
      total_supply = supply;
    ]
    Sell_b -> record [
      token_a_pool = from_pool;
      token_c_pool = to_pool;
      total_supply = supply;
    ]
    Buy_c -> record [
      token_b_pool = from_pool;
...
function form_swap_data(
  ...
  block {
    ...
    const side_c : swap_side = record[
      pool = pair.token_c_pool;
      token = swap.token_c_address;
      id = swap.token_c_id;
      standard = swap.token_c_type;
    ];
  ...
  block {
    ...
    case p of
      AddPair(params) -> {
        ...
        if params.pair.token_a_address = params.pair.token_c_address
        and params.pair.token_a_id >= params.pair.token_c_id
        then failwith("Dex/wrong-token-id") else skip;
        if params.pair.token_a_address > params.pair.token_c_address
        then failwith("Dex/wrong-pair") else skip;

        if params.pair.token_b_address = params.pair.token_c_address
        and params.pair.token_b_id >= params.pair.token_c_id
        then failwith("Dex/wrong-token-id") else skip;
        if params.pair.token_b_address > params.pair.token_c_address
        then failwith("Dex/wrong-pair") else skip;
        ...
        check_token_id(params.pair.token_c_id, params.pair.token_c_type);
        ...
        if pair.token_a_pool * pair.token_c_pool =/= 0n (* no reserves *)
        then failwith("Dex/non-zero-reserves") else skip;

        if pair.token_b_pool * pair.token_c_pool =/= 0n (* no reserves *)
        then failwith("Dex/non-zero-reserves") else skip;

        if pair.total_supply =/= 0n  (* no shares owned *)
        then failwith("Dex/non-zero-shares") else skip;

        if params.token_a_in < 1n && param.token_b_in <1n(* two tokens with 0 *)
        then failwith("Dex/no-token-a") else skip;

        if params.token_b_in < 1n && param.token_c_in <1n(* two tokens with 0 *)
        then failwith("Dex/Not-Sufficent-a&c") else skip;

        if params.token_a_in < 1n && param.token_c_in <1n(* two tokens with 0 *)
        then failwith("Dex/Not-Sufficent-a&c") else skip;
        ...
        pair.token_c_pool := params.token_c_in;
        ...
        operations := 
          typed_transfer(
            ...
            this,
            params.token_c_in,
            params.pair.token_c_id,
            params.pair.token_c_address,
            params.pair.token_c_type
          ) # operations;
      }
    | Swap(n) -> skip
    | Invest(n) -> skip
